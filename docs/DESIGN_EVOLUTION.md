...
E3 (structured summary) ✅ · E4 (Tag + ActionItem + structured `Decision` done;
meeting metadata as real fields done) ✅ · E5 (fonts + face pickers + mood
...
- ✅ `ActionItem` and `Decision` as structured types persisted on the note.
  `Decision` carries text + optional rationale; decodes legacy `[String]`
  decisions for back-compat.
