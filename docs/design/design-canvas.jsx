...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      // Guard against a corrupt/off-screen restore (e.g. a runaway pan): a
      // finite but enormous offset would park all content out of view, leaving
      // a blank canvas. Discard anything that couldn't plausibly be on-screen
      // and fall back to the default {0,0,1} origin.
      const SANE = 200000;
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)
          && Math.abs(s.x) < SANE && Math.abs(s.y) < SANE) {
