...
struct DrawingCanvas: UIViewRepresentable {
    ...
    /// replaced by a clean vector shape (like Apple Notes' shape recognition).
    var recognizeShapes: Bool = true

    /// Called when a stroke was recognized as a shape: the kind plus its rect in
    /// the canvas's *on-screen* coordinates (so the overlay layer can place a
    /// movable/resizable `CanvasItem` exactly where it was drawn).
    ...
    final class Coordinator: NSObject, PKCanvasViewDelegate {
        ...
        /// the freehand ink for a clean vector shape the user can move and resize.
        func canvasViewDidEndUsingTool(_ canvasView: PKCanvasView) {
            defer { lastStrokeCount = canvasView.drawing.strokes.count }
            guard parent.recognizeShapes else { return }
            let strokes = canvasView.drawing.strokes
            // Only a single, freshly-added stroke is a shape candidate (skip erases
            // and multi-stroke changes).
...
enum ShapeRecognizer {
    static func classify(_ stroke: PKStroke) -> (CanvasItem.Kind, CGRect)? {
        ...
        guard diag > 60, w > 28, h > 28 else { return nil }

        // Must be roughly closed (end returns near start) to be a rect/oval.
        let first = pts.first!, last = pts.last!
        guard hypot(last.x - first.x, last.y - first.y) < diag * 0.22 else { return nil }
        ...
        ellErr /= n; rectErr /= n
        // Require a genuinely clean trace; otherwise keep the freehand ink.
        guard min(ellErr, rectErr) < 0.22 else { return nil }
